...
columns = [f'emb{i+1}' for i in range(32)]
embeddings_csv = pd.DataFrame(embeddings, columns = columns)
df_for_names = pd.read_csv('Data/Train_emb.csv').iloc[:5] ###########
names = df_for_names[['name']]
artist = df_for_names[['artist']]
embeddings_csv.insert(0, 'song_name', names)
embeddings_csv.insert(1, 'artist', artist)
np.random.seed(42)
torch.manual_seed(42)

random_10 = embeddings_csv.sample(n = 2, random_state = 42)
remaining_songs = embeddings_csv.drop(random_10.index)

print(random_10.head())
print(remaining_songs.head())

similarities = cosine_similarity(random_10.iloc[:, 2:], remaining_songs.iloc[:, 2:])

top_similar = {}
cosine_values = {}

for i, random_song in enumerate(random_10['song_name']):
...
top_similar_songs_df.index = [f'Top {i+1}' for i in range(3)]
top_similar_songs_df.to_csv('Top_10_songs', index = False)

cosine_values_df = pd.DataFrame(cosine_values)
cosine_values_df.index = [f'Cosine Similarity {i+1}' for i in range(3)]
